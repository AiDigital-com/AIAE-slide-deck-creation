...
| `@RestController` in `service/` | `service` doesn't depend on `spring-web`. Compile error on `@RestController`. | `application/controllers/<aggregate>/<X>Controller.java` |
| Controller directly under `application/` or under an `application/<aggregate>/` folder (e.g. `application/sheets/SheetsProxyController.java`) instead of `application/controllers/<aggregate>/<X>Controller.java` | Breaks the "plural = collections" folder contract; future second controller for the same aggregate has nowhere consistent to land. Past sessions then "fix" by adding it next to the first — spreading inconsistency. | `application/controllers/<aggregate>/<X>Controller.java` (plural collection folder first, aggregate namespace inside it — symmetric with `application/mappers/<aggregate>/`) |
...
# Controllers placed outside the controllers/ folder
...
application/src/main/java/<base>/
  ...
  controllers/                                      # ALL API controllers live here
    <aggregate>/<X>Controller.java                  # implements the generated <Tag>Api
