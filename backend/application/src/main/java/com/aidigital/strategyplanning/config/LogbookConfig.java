package com.aidigital.strategyplanning.config;
 ...
 *
 * <p>Logbook's own defaults are permissive, so this configuration enforces the company rules
 * instead: Authorization and cookie headers masked, secret-like JSON fields masked, actuator /
 * swagger / spec / health / OPTIONS traffic excluded, and metadata-only output so uploaded
 * documents, drafted deck content, and JWTs never reach a log line. Canonical spec:
 * {@code .claude/agent_docs/observability/logbook-http-logging-rules.md}.
 */
@Configuration
